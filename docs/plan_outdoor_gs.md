...
第二の塊は ROS 2 live mapping(`104559a`)。`gs-mapper-live-mapper` node(コアは rclpy-free な `gs_sim2real/robotics/live_mapping.py`)がカメラ topic を keyframe gate に通し、background thread で DUSt3R + gsplat の draft rebuild round を回して `live/latest.splat` を atomic に置き換える。`docs/splat.html` の `?refresh=` polling で browser が地図を in-place 更新する。詳細契約は `docs/live-mapping.md`。
...
同日、リポジトリ名を **`rsasaki0109/3dgs-robotics`** にリネームした(`gs-mapper` → `3dgs-mapper` → `3dgs-robotics`。robotics 応用 = mapping / localization / simulation を名前で見せるユーザー判断。GitHub/PyPI とも空きを確認済み)。GitHub の旧名 URL はリダイレクトされるが **Pages はリダイレクトされない**ため、README 等に残っていた `github.io/gs-mapper` 系リンクは 404 だった。リンクは全面的に新名称へ修正済み(`pages.yml` の `DREAMWALKER_BASE`、Colab の clone/cd、launch-kit はジェネレータ `scripts/generate_launch_kit.py` から再生成)。**据え置いたもの**: HF Space ID `rsasaki0109/gs-mapper`(HF 側の名前空間)、CLI コマンド / pyproject の dist 名 `gs-mapper`(統一リネームは未決定の別案件)、`docs/archive/`(スナップショットのため不変)。注意: push して `pages.yml` が再デプロイされるまで、公開中の Dynamic Map Viewer は旧ベースパス(`/gs-mapper/dreamwalker/`)焼き込みのまま。
...
| `104559a` | ROS 2 live 3DGS mapping(`gs-mapper-live-mapper`、rclpy-free core、`live/latest.splat` atomic swap、polling viewer)。 |
...
- **HF Spaces zero-install demo** `rsasaki0109/gs-mapper` が RUNNING(cpu-basic、gradio 5.49.1 / Python 3.10 / torch 2.4.1 + gsplat pt24cu121)。`apps/hf-space/` から `sync-hf-space.yml` で同期。ZeroGPU 切替はユーザー判断で skip(2026-06-10)。
- **Colab notebook** がある(README badge から)。
- **ROS 2 live mapping**(`gs-mapper-live-mapper`)と実データ growth GIF(`docs/images/live-mapping/live-mapping-grow.gif`、KITTI drive 0056)が README / `docs/live-mapping.md` に載っている。
...
| 候補 | 内容 | 判断 |
| --- | --- | --- |
| 1 | PyPI 公開(`pip install gs-mapper`) | **一旦 skip**(ユーザー判断 2026-06-10)。名前 `gs-mapper` は PyPI 上でまだ空いていることを確認済み。§18.5 参照 |
...
**ゴール**: `gs-mapper map my_drive.mp4` 相当の 1 コマンドで、mp4 → フレーム抽出 → pose-free 復元 → gsplat training → `.splat` export → ローカル viewer 起動までを一気通貫にする。
...
1. `gs-mapper preprocess --method vggt` で KITTI / Bag6 フレームから sparse が出て、既存 train path で `.splat` まで通る。
...
2. CLI(`gs-mapper localize --map <session-dir> --query <image|dir>`)+ KITTI MVP 実走 + 結果 GIF。
3. (任意)ROS 2 node 化(`gs-mapper-live-localizer`、pose を topic で publish)— ここまでやると robotics 文脈で完結する。
...
**ゴール**: ROS 2 ランタイムを立てずに、rosbag2(`.db3` / `.mcap`)を `gs-mapper` に直接食わせて live mapping replay / splat 生成ができる。現状は `ros2 bag play` + live mapper node という「ROS 環境必須」の入口しかなく(`docs/live-mapping.md` 参照)、「ロボットのログがある人」への摩擦が大きい。
...
- 入口は 2 つ:
  ...
  2. `gs-mapper map my_drive.bag`(または既存 video-to-splat 系 subcommand の入力判別)で、bag → フレーム抽出 → 既存 photos-to-splat パスへの thin 配線。`edb109d` の動画ワンライナーと同じ「新しい reconstruction logic は書かない」原則。
...
2. `gs-mapper map <bag>` で ROS なし環境から `.splat` まで通る。
