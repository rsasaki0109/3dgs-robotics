...
2026-06-10 時点では、これに **star growth track** が並走している(§1.3 / §18)。zero-install demo(HF Space / Colab)、ROS 2 live mapping、実データ growth GIF、README 整理までが完了し、告知はユーザーが実行する。開発側の次の弾は §18 の順で、(a) 動画 1 本 → 地図のワンライナー、(b) VGGT feedforward backend、(c) 3DGS 地図に対する localization。Dynamic Map Viewer runtime の信頼性タスク(§1.2 の DoD)は無効になったわけではなく、star growth track と交互に進める。どちらを優先するかはユーザーのその日の指示に従う。
...
2026-06-10 追記: 本 roadmap と並走する形で **§18 の star growth 開発ロードマップ**(動画ワンライナー → VGGT backend → 3DGS localization)が走る。どちらの track を先に進めるかはセッションごとのユーザー指示に従う。本表の優先順位自体は据え置き。
...
| 2 | 動画 1 本 → 地図のワンライナー | **採用**。最初に着手 |
| 3 | VGGT feedforward backend | **採用**。2 の次 |
| 4 | 3DGS 地図に対する localization | 設計まで合意。MVP 定義済み(§18.4)。着手時期は 2/3 完了後に再判断 |
